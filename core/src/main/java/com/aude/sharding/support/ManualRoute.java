package com.aude.sharding.support;
...
public class ManualRoute {
    ...
    public static List<TargetTableEntity> route(String tableName, Map<String, Object> parameters){
        PartitionTable table = shardingContext.getPartitionTable(tableName);
        if (table == null){
            return null;
        }
    ...
    public static List<TargetTableEntity> getAllTables(String tableName){
        return shardingContext.getPartitionTable(tableName).getAllTableNames();
    }
