...
import com.aude.sharding.router.model.TargetTableEntity;
...
import com.aude.sharding.utils.PrintUtil;
        ...
        for (String tableName : tables){
            if (shardingContext.isPartitionTable(tableName)){
                if (partitionTable != null){
                    throw new shardingException("sql中包含了两个分区表");
                }else {
                    partitionTable = shardingContext.getPartitionTable(tableName);
                }
            }
        }
        // 没有规则 无需路由
